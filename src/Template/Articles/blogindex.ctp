...
<script>
    ...
    $(function () {
        ...
        //toggles edit comment forms
        $('.edit-comment').on('click', function () {
            var data = $(this).attr('id').split('-');
            var form = $('#form-edit-' + data[2] + '');
            if (!(form.attr('style') == 'display: none;')) {
                form.addClass('hideit').hide('slow');
            } else {
                form.removeClass('hideit').show('slow');
    ...
    function submitEdit(commentId) {
        ...
        $.ajax({
            type: 'POST',
            data: data,
            url: '<?= $this->Url->build(["controller" => "Comments", "action" => "edit"])?>' + '/' + commentId,
            success: function () {
                location.reload();
            }
        })
    ...
    function submitDelete(commentId) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= $this->Url->build(["controller" => "Comments", "action" => "delete"])?>' + '/' + commentId,
            success: function () {
                location.reload();
            }
        })
